 ...
 * functions for seamless-embeded-posting.
 */
/*global Embeded */
...
(function () {
   ...
   */
  Embeded.util.css = function (elem, property, numeric) {
   ...
   */
  Embeded.util.offsetParent = function (elem) {
    var offsetParent = elem.offsetParent || document.documentElement;
    while (offsetParent && (offsetParent.nodeName !== 'HTML' && Embeded.util.css(offsetParent, 'position') === 'static')) {
   ...
   */
  Embeded.util.offset = function (elem) {
   ...
   * @return {Object} An object containing the properties top and left.
   */
  Embeded.util.position = function (elem) {
    ...
    if (Embeded.util.css(elem, 'position') === 'fixed') {
      // We assume that getBoundingClientRect is available when computed position is fixed
      offset = elem.getBoundingClientRect();
    } else {
      // Get *real* offsetParent
      offsetParent = Embeded.util.offsetParent(elem);

      // Get correct offsets
      offset = Embeded.util.offset(elem);
      if (offsetParent.nodeName !== 'HTML') {
        parentOffset = Embeded.util.offset(offsetParent);
      }

      // Add offsetParent borders
      parentOffset.top += Embeded.util.css(offsetParent, 'borderTopWidth', true);
      parentOffset.left += Embeded.util.css(offsetParent, 'borderLeftWidth', true);
    }

    // Subtract parent offsets and element margins
    return {
      top: offset.top - parentOffset.top - Embeded.util.css(elem, 'marginTop', true),
      left: offset.left - parentOffset.left - Embeded.util.css(elem, 'marginLeft', true)
    };
  };
   ...
   */
  Embeded.util.dispatchInjectedKeyboardEvent = function (target, eventType, charCode, modifierKeys) {
   ...
   */
  Embeded.util.dispatchTextEvent = function (target, eventType, char) {
   ...
   */
  Embeded.util.dispatchMouseEvent = function (target, eventType) {
   ...
   * @return {Boolean}
   */
  Embeded.util.isElementEditable = function (element) {
   ...
   * @return {Node}
   */
  Embeded.util.getOutMostTarget = function (target) {
   ...
   */
  Embeded.util.blockWindowSwitchingBlurEvent = function () {
    _blurLockCounter = {
      blur: 2,
      focusout: 1
    };
